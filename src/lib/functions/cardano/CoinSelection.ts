import type {
 ...
 const CoinSelection = {
   ...
   randomImprove: (inputs, outputs, limit, preset = []) => {
     ...
     const _minUTxOValue = BigInt(outputs.len()) * BigInt("34482");
     ...
     for (let i = 0; i < splitOutputsAmounts.length; i++) {
       ...
       const range = {ideal:undefined, maximum:undefined};
     ...
     if (utxoSelection.remaining.length > 0) {
       ...
       const maxFee = cardanoWasm.Value.new(
         cardanoWasm.BigNum.from_str(maxFee_.toString())
       );
 ...
 function descSelect(utxoSelection, outputAmount, limit, minUTxOValue) {
   ...
   do {
     ...
     /** @type {TransactionUnspentOutput} utxo */
     const utxo = utxoSelection.subset.splice(0, 1).pop();
   ...
   if (amounts.multiasset()) {
     ...
     for (let i = 0; i < mA.keys().len(); i++) {
       ...
       for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
         const _assets = cardanoWasm.Assets.new();
         ...
         const _multiasset = cardanoWasm.MultiAsset.new();
         _multiasset.insert(
           cardanoWasm.ScriptHash.from_bytes(scriptHash.to_bytes()),
           _assets
         );
         const _value = cardanoWasm.Value.new(
           cardanoWasm.BigNum.from_str("0")
         );
 ...
 function isQtyFulfilled(
   ...
   if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
     const minAmount = cardanoWasm.Value.new(
       cardanoWasm.min_ada_required(
         cumulatedAmount,
         false,
         cardanoWasm.BigNum.from_str(protocolParameters.coinsPerUtxoWord)
